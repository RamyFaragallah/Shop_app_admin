...
class ColorDialog extends StatefulWidget {
  @override
  _ColorDialogState createState() => new _ColorDialogState();
}
class _ColorDialogState extends State<ColorDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      content:  SingleChildScrollView(
        child:Container(
          ...
          child: MaterialColorPicker(
            circleSize: 70,
            onColorChange: (color){pickedColor=color.toString().substring(6,16);
           },
      ...
      FlatButton(onPressed: (){if(pickedColor!=null){
        selectedColors.add(pickedColor.toString());
        Navigator.pop(context,selectedColors);
        }else{Fluttertoast.showToast(msg: "choolse a color please");}
      }, child: Text('ADD')),

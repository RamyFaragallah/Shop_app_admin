...
class UpdateCATDialog extends StatefulWidget {
  ...
  @override
  _UpdateCATDialogState createState() => new _UpdateCATDialogState(_snapshot);
}
class _UpdateCATDialogState extends State<UpdateCATDialog> {
  ...
  void saveANDupload() {
    if (_CATFormKey.currentState.validate()) {
      if (image1 != null ) {
        ...
        _uploadimages(image1, CAT_name).then((img){
          setState(() {
            imgsurl=img;
            if(imgsurl.isNotEmpty){
              uploadCAT(CAT_name, imgsurl)  ;        setState(() {
                loading=false;
              });
        ...
        uploadCAT(CAT_name, _snapshot["imgurl"])  ;        setState(() {
          loading=false;
        });
        Fluttertoast.showToast(msg: "category updated");
        Navigator.of(context).pop();
  ...
  void uploadCAT(String CAT,String imgsurl){
    ...
    try{
      setState(() {
        _firestore.collection('categories').document(_snapshot.documentID).setData({'category': CAT,"imgurl":imgsurl});
      });
    }catch(e){

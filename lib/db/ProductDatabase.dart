...
class ProductDatabase {
  ...
  Future<List<String>> uploadimages(
      File imgfile1, File imgfile2, File imgfile3, String productName) async {
  ...
  Future<List<DocumentSnapshot>> getallproducts(
    ...
    List<DocumentSnapshot> documentSnapshot;
      QuerySnapshot querytSnapshot =
          await Firestore.instance.collection("products").where("name",isEqualTo: pattern).getDocuments();
      documentSnapshot = querytSnapshot.documents;
  ...
  Future<int> productCount() async {
    int count = 0;
    await getallproducts("All", "All").then((list) {
      count = list.length;
    });
    return count;
  }
